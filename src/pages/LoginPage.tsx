// AllPass/src/pages/LoginPage.tsx
...
const LoginPage: React.FC<LoginPageProps> = ({ darkMode, toggleTheme }) => {
  ...
  const handleContinue = async () => { // Обработчик кнопки "Продолжить"
    ...
    try {
      const response = await fetch(`${API_URL}/api/auth/users/${email}/`); // Запрос к API для проверки пользователя
      const data = await response.json(); // Парсим ответ в JSON
      console.log("Login response:", data); // Логируем ответ для отладки
      ...
      localStorage.setItem("user_id", data.id); // ID пользователя
      localStorage.setItem("user_email", data.email); // Email
      localStorage.setItem("user_first_name", data.first_name); // Имя
      localStorage.setItem("user_family_name", data.family_name); // Фамилия
      localStorage.setItem("user_father_name", data.father_name); // Отчество
      localStorage.setItem("user_phone", data.phone); // Телефон
      localStorage.setItem("user_name", `${data.first_name} ${data.family_name}`); // Полное имя
      ...
      setErrorMessage("Ошибка сети. Попробуйте позже."); // Устанавливаем сообщение об ошибке
    } finally {
