...
const Submit: React.FC<SubmitProps> = ({ darkMode, toggleTheme }) => {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
    ...
    console.log(`Изменение поля ${name}: значение ${value}`); // Логируем каждое изменение
    if (["latitude", "longitude", "height"].includes(name)) {
      ...
      const newValue = parseInt(value); // Преобразуем значение в число
      ...
      setFormData((prev) => {
        ...
        console.log("Обновлённые difficulties:", updatedDifficulties); // Логируем обновлённые данные
        return {
          ...prev,
      ...
      if (name === "difficulty") {
        ...
        console.log("Выбранная сложность:", selected); // Логируем выбранную сложность
